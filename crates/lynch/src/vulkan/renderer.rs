use crate::{Texture , vulkan::cont::*, vulkan::debug::*,  window::window::Window, renderer::Renderer, mesh_loader::Model, Camera};
use ash::{extensions::{
        ext::DebugUtils,
...
impl Renderer for VulkanRenderer {
    ...
    fn add_model(self: &mut Self, model: crate::mesh_loader::Model, transform: glam::Mat4) {
        info!("Adding model.");
    ...
    fn resize(self: &mut Self, resize: winit::dpi::PhysicalSize<u32>) {
        self.surface_resolution = Extent2D{width: resize.width, height: resize.height};
...
impl Drop for VulkanRenderer{
    fn drop(&mut self){
        let command_buffers : Vec<vk::CommandBuffer> = self.sync_frames.iter().map(|f| f.command_buffer ).clone().collect();
